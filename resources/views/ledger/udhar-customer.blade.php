@extends('layouts.app')
@section('title', $udharCustomer->name . ' — Udhar Ledger')
@section('content')
<div class="space-y-5">

  <div class="flex items-center gap-3">
    <a href="{{ route('udhar-customers.show', $udharCustomer) }}" class="w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:text-slate-700 shadow-sm">
      <i data-lucide="arrow-left" class="w-4 h-4"></i>
    </a>
    <div>
      <h1 class="text-xl font-bold text-slate-900">{{ $udharCustomer->name }} — Udhar Ledger</h1>
      @if($udharCustomer->notes)
        <p class="text-sm text-slate-500">{{ $udharCustomer->notes }}</p>
      @endif
    </div>
    <div class="ml-auto">
  ...
  <div class="grid grid-cols-3 gap-4">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 uppercase tracking-wider font-medium">Total Udhar</p>
  ...
  <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100">
      <h2 class="font-semibold text-slate-900">Udhar History</h2>
    </div>
      ...
      <table class="w-full">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Udhar (Debit)</th>
            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Payment (Credit)</th>
