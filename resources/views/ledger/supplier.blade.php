...
<div class="space-y-5">
  ...
  <div class="flex items-center gap-3">
    <a href="{{ route('suppliers.show', $supplier) }}" class="w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:text-slate-700 shadow-sm">
      <i data-lucide="arrow-left" class="w-4 h-4"></i>
    </a>
    <div>
      <h1 class="text-xl font-bold text-slate-900">{{ $supplier->name }} — Ledger</h1>
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 uppercase tracking-wider font-medium">Opening Balance</p>
      <p class="text-xl font-bold text-slate-700 mt-1">PKR {{ number_format($openingBalance) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 uppercase tracking-wider font-medium">Total Purchases</p>
      <p class="text-xl font-bold text-red-600 mt-1">PKR {{ number_format($totalDebit) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 uppercase tracking-wider font-medium">Total Paid / Returns</p>
      ...
      <table class="w-full">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Debit (Owe)</th>
            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Credit (Paid)</th>
