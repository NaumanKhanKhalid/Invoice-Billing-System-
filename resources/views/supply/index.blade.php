  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Today's Supply</p>
        <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center">
          <i data-lucide="package" class="w-4 h-4 text-slate-500"></i>
        </div>
      </div>
      <p class="text-xl font-bold text-slate-900">{{ formatCurrency($stats['total_today']) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-orange-200 shadow-sm p-5">
      ...
      <p class="text-xl font-bold text-orange-600">{{ formatCurrency($stats['total_due']) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-red-200 shadow-sm p-5">
      ...
      <p class="text-xl font-bold text-red-600">{{ $stats['overdue_count'] }} orders</p>
    </div>
    <div class="bg-white rounded-xl border border-green-200 shadow-sm p-5">
      <div class="flex items-center justify-between mb-3">
        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">This Month (kg)</p>
        <div class="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center">
          <i data-lucide="scale" class="w-4 h-4 text-green-600"></i>
        </div>
      </div>
      <p class="text-xl font-bold text-green-600">{{ formatKg($stats['this_month_kg']) }}</p>
    </div>
  </div>
    ...
    <form method="GET" class="flex flex-wrap gap-3 items-center">
      <div class="relative">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Invoice #..."
               class="pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white w-36">
      </div>
      <div class="relative">
        <i data-lucide="users" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"></i>
        <select name="customer_id" class="pl-9 pr-8 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white text-slate-700 appearance-none cursor-pointer">
      ...
      @if(request()->hasAny(['search','customer_id','status','from_date','to_date']))
      <a href="{{ route('supply.index') }}" class="flex items-center gap-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 px-4 py-2 rounded-lg text-sm transition-colors">
