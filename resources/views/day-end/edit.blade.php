@extends('layouts.app')
@section('title','Edit Day Record — ' . \Carbon\Carbon::parse($dayEnd->date)->format('d M Y'))
@section('content')
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      <a href="{{ route('day-end.show', $dayEnd) }}" class="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-slate-700 transition-colors">
        <i data-lucide="arrow-left" class="w-4 h-4"></i>
      </a>
      <div>
        <h1 class="text-xl font-bold text-slate-900">Edit Day Record</h1>
        <p class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($dayEnd->date)->format('l, d M Y') }}</p>
      </div>
    </div>
  </div>
  ...
  <form method="POST" action="{{ route('day-end.update', $dayEnd) }}" class="space-y-4">
    @csrf @method('PUT')
        ...
        <div class="p-4 space-y-3">
          <div>
            ...
            <input type="number" name="opening_stock_live_kg"
                   value="{{ old('opening_stock_live_kg', $dayEnd->opening_stock_live_kg) }}"
          ...
          <div>
            ...
            <input type="number" name="opening_stock_dressed_kg"
                   value="{{ old('opening_stock_dressed_kg', $dayEnd->opening_stock_dressed_kg) }}"
                   step="0.001" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
        </div>
        ...
        <div class="p-4 space-y-3">
          <div>
            ...
            <input type="number" name="total_purchased_live_kg"
                   value="{{ old('total_purchased_live_kg', $dayEnd->total_purchased_live_kg) }}"
                   step="0.001" min="0" required
          ...
          <div>
            ...
            <input type="number" name="purchase_cost"
                   value="{{ old('purchase_cost', $dayEnd->purchase_cost) }}"
        ...
        <div class="p-4 space-y-3">
          <div>
            ...
            <input type="number" name="total_supply_dressed_kg"
                   value="{{ old('total_supply_dressed_kg', $dayEnd->total_supply_dressed_kg) }}"
                   step="0.001" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div>
            ...
            <input type="number" name="total_supply_revenue"
                   value="{{ old('total_supply_revenue', $dayEnd->total_supply_revenue) }}"
                   step="0.01" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
        </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="flex items-center gap-2 px-4 py-3 border-b border-yellow-100 bg-yellow-50">
          ...
          <div>
            ...
            <p class="text-xs text-yellow-600">Cash collected at counter</p>
          </div>
        </div>
        <div class="p-4">
          ...
          <div class="relative">
            ...
            <input type="number" name="counter_cash"
                   value="{{ old('counter_cash', $dayEnd->counter_cash) }}"
                   step="0.01" min="0" required
        ...
        <div class="p-4 grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" name="closing_stock_live_kg"
                   value="{{ old('closing_stock_live_kg', $dayEnd->closing_stock_live_kg) }}"
                   step="0.001" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div>
            ...
            <input type="number" name="closing_stock_dressed_kg"
                   value="{{ old('closing_stock_dressed_kg', $dayEnd->closing_stock_dressed_kg) }}"
                   step="0.001" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div>
            ...
            <input type="number" name="closing_stock_value"
                   value="{{ old('closing_stock_value', $dayEnd->closing_stock_value) }}"
                   step="0.01" min="0" required
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
        </div>
    ...
    <div class="grid grid-cols-3 gap-4">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        ...
        <div class="space-y-3">
          <div>
            ...
            <input type="number" name="dead_kg" value="{{ old('dead_kg', $dayEnd->dead_kg) }}" step="0.001" min="0"
          ...
          <div>
            ...
            <input type="number" name="spoilage_kg" value="{{ old('spoilage_kg', $dayEnd->spoilage_kg) }}" step="0.001" min="0"
                   class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-200 outline-none">
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        ...
        <div class="relative">
          ...
          <input type="number" name="total_expenses"
                 value="{{ old('total_expenses', $dayEnd->total_expenses) }}"
                 step="0.01" min="0" required
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        ...
        <textarea name="notes" rows="4" placeholder="Any special notes..."
                  class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none resize-none">{{ old('notes', $dayEnd->notes) }}</textarea>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between pt-1 pb-6">
      <a href="{{ route('day-end.show', $dayEnd) }}" class="text-sm text-slate-500 hover:text-slate-700">Cancel</a>
      <button type="submit"
