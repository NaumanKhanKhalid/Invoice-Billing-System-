...
<div class="max-w-3xl mx-auto space-y-5">
  ...
  <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="bg-slate-800 px-5 py-3">
      <h2 class="text-white font-semibold">Day End — {{ \Carbon\Carbon::parse($date)->format('d M Y') }}</h2>
    </div>
      ...
      <div class="bg-slate-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="opening_stock_live_kg" value="{{ old('opening_stock_live_kg', $prevRecord?->closing_stock_live_kg ?? 0) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div>
            <label class="block text-xs text-slate-500 mb-1">Dressed Stock (kg)</label>
            <input type="number" name="opening_stock_dressed_kg" value="{{ old('opening_stock_dressed_kg', $prevRecord?->closing_stock_dressed_kg ?? 0) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
        </div>
      </div>
      ...
      <div class="bg-blue-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="total_purchased_live_kg" value="{{ old('total_purchased_live_kg', $purchaseLiveKg) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
          </div>
          <div>
            <label class="block text-xs text-slate-500 mb-1">Purchase Cost (PKR)</label>
            <input type="number" name="purchase_cost" value="{{ old('purchase_cost', $purchaseCost) }}" step="0.01" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
          </div>
        </div>
      </div>
      ...
      <div class="bg-green-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="total_supply_dressed_kg" value="{{ old('total_supply_dressed_kg', $supplyKg) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
          </div>
          <div>
            <label class="block text-xs text-slate-500 mb-1">Supply Revenue (PKR)</label>
            <input type="number" name="total_supply_revenue" value="{{ old('total_supply_revenue', $supplyTotal) }}" step="0.01" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
          </div>
        </div>
      </div>
      ...
      <div class="bg-yellow-50 rounded-lg p-4">
        ...
        <div>
          ...
          <div class="relative max-w-xs">
            ...
            <input type="number" name="counter_cash" value="{{ old('counter_cash', 0) }}" step="0.01" min="0" required class="w-full pl-12 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
          </div>
        </div>
      </div>
      ...
      <div class="bg-slate-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="closing_stock_live_kg" value="{{ old('closing_stock_live_kg', 0) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div>
            <label class="block text-xs text-slate-500 mb-1">Dressed Stock Remaining (kg)</label>
            <input type="number" name="closing_stock_dressed_kg" value="{{ old('closing_stock_dressed_kg', 0) }}" step="0.001" min="0" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
          </div>
          <div class="sm:col-span-2">
            <label class="block text-xs text-slate-500 mb-1">Closing Stock Value (PKR)</label>
            <div class="relative max-w-xs">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">PKR</span>
              <input type="number" name="closing_stock_value" value="{{ old('closing_stock_value', 0) }}" step="0.01" min="0" required class="w-full pl-12 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
            </div>
          </div>
        </div>
      </div>
