  ...
  <div x-show="finalizeModalOpen" x-cloak x-transition.opacity
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full overflow-hidden flex flex-col max-h-[88vh]" style="max-width:23rem"
      ...
      <div class="flex items-start justify-between px-4 py-3 border-b border-slate-100 shrink-0">
        <div>
          <h3 class="font-bold text-slate-900 leading-tight">{{ __('pos.payment') }}</h3>
          <p class="text-xs text-slate-400 mt-0.5">{{ __('pos.payment_subtitle') }}</p>
        </div>
        <button type="button" @click="finalizeModalOpen = false" class="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 shrink-0">✕</button>
      </div>

      <div class="overflow-y-auto">
      {{-- Total --}}
      <div class="px-4 pt-2.5">
        <div class="flex items-center justify-between rounded-xl px-4 py-3" style="background:linear-gradient(135deg,#f0fdf4 0%,#ecfdf5 100%)">
          <span class="text-sm text-green-700/70 font-semibold">{{ __('pos.total_to_pay') }}</span>
          <span class="text-2xl font-extrabold text-green-700 tabular-nums" x-text="'PKR ' + total.toLocaleString()"></span>
        </div>
      </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0.375rem">
          <template x-for="pm in payMethods" :key="pm.value">
            <button type="button" @click="payMethod = pm.value"
                    :class="payMethod === pm.value ? 'border-green-500 bg-green-50 text-green-700 ring-1 ring-green-200' : 'border-slate-200 bg-white text-slate-500 hover:border-green-300'"
      ...
      <div class="px-4 py-3 shrink-0 border-t border-slate-100 bg-slate-50/50">
        ...
        <div @click="toggleAutoPrint()" role="switch" :aria-checked="autoPrint"
             class="flex items-center justify-between gap-2 mb-2.5 px-3 py-2 rounded-xl bg-white border border-slate-200 cursor-pointer select-none hover:border-green-300 transition">
        ...
        <button type="button" @click="confirmPayment(false)" :disabled="submitting"
                class="w-full bg-green-600 hover:bg-green-700 active:scale-[.99] disabled:bg-slate-200 disabled:text-slate-400 text-white py-3.5 rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow-sm shadow-green-600/25">
