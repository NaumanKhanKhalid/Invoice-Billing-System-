...
<div class="space-y-5">
  <div class="flex items-center justify-between">
    <div><h1 class="text-2xl font-bold text-slate-900">Sales Orders</h1><p class="text-sm text-slate-500 mt-0.5">All chicken sales</p></div>
    <a href="{{ route('sales.create') }}" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"><i data-lucide="plus" class="w-4 h-4"></i>New Sale</a>
  </div>

  <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Today's Sales</p>
      <p class="text-xl font-bold text-slate-900 mt-1">PKR {{ number_format($stats['total_today'],0) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Outstanding</p>
      <p class="text-xl font-bold text-orange-600 mt-1">PKR {{ number_format($stats['total_due'],0) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-center">
      ...
      <p class="text-xl font-bold text-green-600 mt-1">{{ number_format($stats['this_month_kg'],1) }}</p>
    </div>
  </div>
  ...
  <form method="GET" class="flex flex-wrap gap-3">
    ...
    <select name="order_type" class="px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
      <option value="">All Types</option>
      <option value="retail" {{ request('order_type')=='retail'?'selected':'' }}>Retail</option>
      <option value="supply" {{ request('order_type')=='supply'?'selected':'' }}>Supply</option>
    </select>
    ...
    @if(request()->hasAny(['customer_id','order_type','chicken_type_id','status','from_date','to_date']))<a href="{{ route('sales.index') }}" class="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-lg text-sm">Clear</a>@endif
  </form>
    ...
    <table class="w-full">
      ...
      <tbody class="divide-y divide-slate-100">
        ...
        <tr class="hover:bg-slate-50 transition-colors {{ $isOverdue?'bg-red-50':'' }}">
          ...
          <td class="px-4 py-3 text-sm font-medium"><a href="{{ route('sales.show',$order) }}" class="text-slate-900 hover:text-green-600">{{ $order->invoice_number }}</a></td>
          <td class="px-4 py-3 text-sm text-slate-700">{{ $order->customer?->name ?? 'Walk-in' }}</td>
          <td class="px-4 py-3"><span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $order->order_type==='supply'?'bg-blue-100 text-blue-700':'bg-slate-100 text-slate-600' }}">{{ ucfirst($order->order_type) }}</span></td>
          ...
          <td class="px-4 py-3">
            <div class="flex items-center justify-end gap-2">
              <a href="{{ route('sales.show',$order) }}" class="text-slate-400 hover:text-green-600"><i data-lucide="eye" class="w-4 h-4"></i></a>
              @if($order->payment_status!=='paid')<a href="{{ route('sales.edit',$order) }}" class="text-slate-400 hover:text-blue-600"><i data-lucide="pencil" class="w-4 h-4"></i></a>@endif
              @if($order->payment_status==='unpaid')<form method="POST" action="{{ route('sales.destroy',$order) }}" class="inline" onsubmit="return confirm('Delete this sale?')">@csrf @method('DELETE')<button type="submit" class="text-slate-400 hover:text-red-600"><i data-lucide="trash-2" class="w-4 h-4"></i></button></form>@endif
            </div>
          </td>
        </tr>
        @empty
        <tr><td colspan="9" class="px-4 py-12 text-center">
          ...
          <a href="{{ route('sales.create') }}" class="text-green-600 text-sm mt-1 inline-block hover:underline">Record first sale</a>
        </td></tr>
        @endforelse
      </tbody>
    </table>
