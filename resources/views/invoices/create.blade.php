...
<div class="max-w-5xl mx-auto space-y-5">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900">New Invoice</h1>
            <p class="text-sm text-slate-500 mt-0.5">Invoice # <strong>{{ $invoiceNumber }}</strong></p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('invoices.store') }}" id="invoice-form">
        ...
        <div x-data="invoiceForm(JSON.parse(document.getElementById('products-data').textContent))" class="space-y-5">
        <script id="products-data" type="application/json">@json($products->values())</script>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="client_id" required
                            class="w-full px-3 py-2 text-sm border @error('client_id') border-red-400 @else border-slate-200 @enderror rounded-lg focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none bg-white">
                ...
                <div>
                    ...
                    <input type="date" name="issue_date" value="{{ old('issue_date', date('Y-m-d')) }}" required
                           class="w-full px-3 py-2 text-sm border @error('issue_date') border-red-400 @else border-slate-200 @enderror rounded-lg focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none">
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d', strtotime('+30 days'))) }}" required
                           class="w-full px-3 py-2 text-sm border @error('due_date') border-red-400 @else border-slate-200 @enderror rounded-lg focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none">
                                ...
                                <tr class="border-b border-slate-50">
                                    <td class="px-4 py-2">
                                        <input type="text" :name="`items[${index}][description]`" x-model="item.description" required
                                               class="w-full px-2 py-1.5 text-sm border border-slate-200 rounded focus:ring-1 focus:ring-indigo-300 focus:border-indigo-400 outline-none"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                    ...
                    <textarea name="terms" rows="3"
                              class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-300 outline-none resize-none"
                              placeholder="Payment is due within 30 days...">{{ old('terms') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 pb-6">
                <button type="submit" name="action" value="draft"
                        class="inline-flex items-center gap-2 bg-slate-700 hover:bg-slate-800 text-white font-medium px-5 py-2.5 rounded-lg transition-colors text-sm">
