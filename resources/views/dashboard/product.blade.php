...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between flex-wrap gap-3">
    ...
    <div class="flex gap-2">
      <a href="{{ route('pos.create') }}" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        <i data-lucide="scan-line" class="w-4 h-4"></i>New Sale
      </a>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      ...
      <p class="text-xl font-bold text-{{ $monthProfit >= 0 ? 'green-700' : 'red-700' }}">{{ formatCurrency(abs($monthProfit)) }}</p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-5 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors">
        <div>
          <p class="text-sm font-semibold text-green-600">{{ $s->sale_number }}</p>
          <p class="text-xs text-slate-400">{{ $s->customer_name ?: 'Walk-in' }} · {{ \Carbon\Carbon::parse($s->date)->format('d M, h:i A') }}</p>
        </div>
        <div class="text-right">
          <p class="text-sm font-bold text-slate-900">{{ formatCurrency($s->total) }}</p>
          <p class="text-xs text-slate-400 capitalize">{{ $s->payment_method }}</p>
        </div>
      </div>
      @empty
