...
<div class="max-w-3xl mx-auto space-y-5" x-data="{
    ...
    doa: {{ $purchase->dead_on_arrival_kg ?? 0 }},
    ...
    setRateFromType(typeId) {
        ...
        fetch('{{ route('daily-rates.today') }}')
            ...
            .then(rates=>{
                ...
                if(rate) this.ratePerKg = rate.rate_per_kg;
            });
    }
}">
  <div class="flex items-center gap-3"><a href="{{ route('purchases.show',$purchase) }}" class="text-slate-400 hover:text-slate-600"><i data-lucide="arrow-left" class="w-5 h-5"></i></a><div><h1 class="text-2xl font-bold text-slate-900">Edit Purchase Order</h1><p class="text-sm text-slate-500">{{ $purchase->invoice_number }}</p></div></div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          <select name="supplier_id" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none bg-white">
            ...
            @foreach($suppliers as $s)<option value="{{ $s->id }}" {{ (old('supplier_id',$purchase->supplier_id)==$s->id)?'selected':'' }}>{{ $s->name }} ({{ $s->credit_days }}d)</option>@endforeach
          </select>
          @error('supplier_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Date <span class="text-red-500">*</span></label>
          <input type="date" name="date" value="{{ old('date', $purchase->date) }}" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none">
        </div>
        <div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div>
          ...
          <input type="number" name="dressed_weight_kg" x-model="dressedKg" value="{{ old('dressed_weight_kg',$purchase->dressed_weight_kg) }}" step="0.001" min="0.001" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Dead on Arrival (kg)</label>
          <input type="number" name="dead_on_arrival_kg" x-model="doa" value="{{ old('dead_on_arrival_kg',$purchase->dead_on_arrival_kg??0) }}" step="0.001" min="0" class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none">
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-slate-50 rounded-lg p-3 text-center">
          <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Waste</p>
          <p class="text-xl font-bold text-slate-700 mt-1" x-text="wasteKg.toFixed(1) + ' kg'">0.0 kg</p>
        </div>
        <div class="bg-slate-50 rounded-lg p-3 text-center">
          <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Yield %</p>
          <p class="text-xl font-bold mt-1" :class="yieldPct<65||yieldPct>80?'text-red-600':'text-green-600'" x-text="yieldPct + '%'">0%</p>
          <p class="text-xs text-slate-400 mt-0.5">Normal: 68-75%</p>
        </div>
        <div class="bg-green-50 rounded-lg p-3 text-center border border-green-100">
          <p class="text-xs text-green-600 font-medium uppercase tracking-wider">Total Amount</p>
          <p class="text-xl font-bold text-green-700 mt-1" x-text="'PKR ' + parseFloat(totalAmount).toLocaleString('en-PK',{minimumFractionDigits:0})">PKR 0</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          <div class="relative">
            ...
            <input type="number" name="rate_per_kg_live" x-model="ratePerKg" value="{{ old('rate_per_kg_live',$purchase->rate_per_kg_live) }}" step="0.01" min="0" required class="w-full pl-12 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none">
          </div>
        </div>
        <div>
