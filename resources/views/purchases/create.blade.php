...
<div class="max-w-5xl mx-auto space-y-6" x-data="{
  ...
  <div class="flex items-center gap-3">
    <a href="{{ route('purchases.index') }}" class="w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
      <i data-lucide="arrow-left" class="w-4 h-4"></i>
    </a>
    <div>
      <h1 class="text-xl font-bold text-slate-900">New Purchase Order</h1>
      <p class="text-sm text-slate-500">Record chicken purchase from supplier</p>
    </div>
  </div>
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      <div class="col-span-2 space-y-5">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div class="p-5 space-y-4">
              ...
              <div>
                ...
                <select name="supplier_id" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none bg-white">
                  ...
                  @foreach($suppliers as $s)
                    <option value="{{ $s->id }}" {{ old('supplier_id')==$s->id?'selected':'' }}>{{ $s->name }} ({{ $s->credit_days }}d)</option>
                  @endforeach
                </select>
            ...
            <div>
              <label class="block text-xs font-medium text-slate-500 mb-1.5">Invoice # <span class="text-slate-400">(auto-generated)</span></label>
          ...
          <div class="p-5 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="live_weight_kg" x-model="liveKg" value="{{ old('live_weight_kg') }}" step="0.001" min="0.001" required class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
              ...
              <div>
                ...
                <input type="number" name="dead_on_arrival_kg" x-model="doa" value="{{ old('dead_on_arrival_kg', 0) }}" step="0.001" min="0" class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
              ...
              <div class="col-span-2">
                ...
                <div class="relative">
                  ...
                  <input type="number" name="rate_per_kg_live" x-model="ratePerKg" value="{{ old('rate_per_kg_live') }}" step="0.01" min="0" required class="w-full pl-14 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-300 outline-none">
                </div>
      ...
      <div class="space-y-4">
        ...
        <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors">
          <i data-lucide="check-circle" class="w-4 h-4"></i> Save Purchase Order
        </button>
        <a href="{{ route('purchases.index') }}" class="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center transition-colors">
          Cancel
        </a>
